...
// FIX: Form validation now properly shows inline errors instead of alerts
// - Negative numbers and empty titles are blocked with clear inline error messages
// - Submit is disabled when there are validation errors
...
export function CreateBountyForm({ onSubmit }: CreateBountyFormProps) {
  ...
  const validate = () => {
    ...
    if (!reward.trim() || isNaN(rewardNum) || rewardNum <= 0) {
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div>
          ...
          <input
            ...
            className={`w-full rounded-lg border px-3 py-2 ${errors.title ? 'border-red-400 bg-red-50' : 'border-slate-200'}`}
        ...
        <div>
          ...
          <input
            ...
            className={`w-full rounded-lg border px-3 py-2 ${errors.reward ? 'border-red-400 bg-red-50' : 'border-slate-200'}`}
            placeholder="100"
            min="1"
          />
          {errors.reward && (
