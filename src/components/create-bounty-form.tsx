...
export function CreateBountyForm({ onSubmit }: CreateBountyFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Keep validation inline so the form remains usable without blocking alerts.
      const nextErrors: { title?: string; reward?: string } = {};
      if (!title.trim()) nextErrors.title = "Title is required";
      const rewardNum = Number(reward);
      if (isNaN(rewardNum) || rewardNum <= 0) nextErrors.reward = "Reward must be a positive number";
